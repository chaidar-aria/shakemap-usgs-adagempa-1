...
def main():
    ...
    if args.preferred_eventsource in ("pt", "at", "dr", "ew"):
        logger.info(f"Undesirable eventsource: {args.preferred_eventsource}")
        sys.exit(0)

    if args.status == "DELETE":
        event = {
            "id": args.preferred_eventid,
            "netid": args.preferred_eventsource,
            "alt_eventids": args.eventids,
        }
        queue.send_queue("cancel", event, config["port"])
        logger.info(f"Sending cancel event {args.preferred_eventid} to queue.")
        sys.exit(0)

    if (
        not args.preferred_eventtime
        or not args.preferred_latitude
        or not args.preferred_longitude
        or not args.preferred_depth
    ...
    while fails < 3:
        try:
            detail = get_detail_json(args.preferred_eventid)
    ...
    if fails == 3:
        logger.warn(
            "Unable to retrieve event data from comcat for event %s"
            % (args.preferred_eventid)
        )
        dt = datetime.strptime(args.preferred_eventtime, constants.TIMEFMT)
        ...
        if event_age > 86400:
            ...
            logger.warn(
                f"Event {args.preferred_eventid} is older than cutoff, skipping"
            )
            sys.exit(0)
        else:
            # Let's run the event
        ...
        if detail["id"] != args.preferred_eventid:
            logger.info(
                "Event id %s is not authoritative; (authoritative=%s)"
    ...
    if args.property_review_status != "reviewed":
        logger.info(
            ...
            % (args.preferred_eventid, args.property_review_status)
        )
        sys.exit(0)
    ...
    if not args.property_title:
        ...
        while fails < 3:
            try:
                gs = GeoServe(
                    args.preferred_latitude,
                    args.preferred_longitude,
                    maxradius=250,
                    minpop=1000,
                )
            except Exception as e:
    ...
    logger.info(f"Sending event {args.preferred_eventid} to queue.")
